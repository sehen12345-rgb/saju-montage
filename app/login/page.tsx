...
function LoginContent() {
  ...
  useEffect(() => {
    fetch("/api/auth/providers")
      .then((r) => r.json())
      .then((data) => {
        setAvailableProviders(Object.keys(data ?? {}));
  ...
  async function handleSocialLogin(provider: string) {
    if (!availableProviders.includes(provider)) {
      showToast("현재 이 로그인 방법은 준비 중입니다. 이메일로 로그인해주세요.");
      return;
    }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-10">
      {/* 토스트 */}
      {toast && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 bg-gray-900 text-white text-sm rounded-2xl px-5 py-3 shadow-xl max-w-xs text-center">
          {toast}
        </div>
      )}
